...
and expr_node =
  ...
  | Postincr of access (* Post-increment operation: x++ *)
  | Postdecr of access (* Post-decrement operation: x-- *)
  | BinaryOpEq of binop * access * expr (* Abbreviation for assignment ops: +=, -=, ... *)
[@@deriving show]
...
and stmtordec_node =
  | Dec of ((typ * identifier) * (expr list)) list (* Local variable declaration *)
...
and topdecl_node = 
  | Fundecl of fun_decl 
  (* simple dec; dec with init (also for array); multiple dec with init *)
